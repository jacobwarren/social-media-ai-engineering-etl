...
def create_prompt(post, include_topic=True):
    """
    Create prompt with or without topic feature
    ...
    # Only difference is whether we include specific topic or use generic wording
    if include_topic and 'topic' in post:
        topic = post.get('topic')
        first_line = f"Create a LinkedIn post on the topic of: `{topic}`"
    else:
        # Generic opening without specific topic
...
def generate_posts(llm, posts, include_topic=True, llm_batch=8, temperature=0.7, max_tokens=1000, top_p=0.9):
    ...
    """
    logger.info(f"Generating posts with topic={'included' if include_topic else 'excluded'}...")
    prompts = [create_prompt(post, include_topic) for post in posts]
...
def compute_embedding_similarities(with_topic_embeddings, without_topic_embeddings):
    """
    ...
    for i in range(min(len(with_topic_embeddings), len(without_topic_embeddings))):
        sim = cosine_similarity(
            with_topic_embeddings[i].reshape(1, -1),
            without_topic_embeddings[i].reshape(1, -1)
        )[0][0]
        similarities.append(sim)
    
    return similarities

def analyze_differences(with_topic_texts, without_topic_texts, posts):
    """
    Analyze differences between texts generated with and without topic
    """
    logger.info("Analyzing text differences...")
    results = []
    
    for i, (with_t, without_t, post) in enumerate(zip(with_topic_texts, without_topic_texts, posts)):
        ...
        results.append({
            'index': i,
            'topic': topic,
            'with_topic_length': with_t_length,
            'without_topic_length': without_t_length,
            'length_diff_pct': length_diff_pct,
            'paragraph_diff': paragraph_diff,
            'bullet_diff': bullet_diff,
            'with_topic_text': with_t[:200] + "..." if len(with_t) > 200 else with_t,
            'without_topic_text': without_t[:200] + "..." if len(without_t) > 200 else without_t
        })
...
def categorize_topic(topic):
    """
    Categorize topics into broader groups for visualization
    """
    topic_lower = topic.lower()
    if any(kw in topic_lower for kw in ['marketing', 'sales', 'branding', 'advertising']):
        return 'Marketing & Sales'
    elif any(kw in topic_lower for kw in ['tech', 'ai', 'software', 'data', 'programming', 'digital']):
        return 'Technology & AI'
    elif any(kw in topic_lower for kw in ['leadership', 'management', 'career', 'professional']):
        return 'Leadership & Career'
    elif any(kw in topic_lower for kw in ['health', 'wellness', 'fitness', 'mental']):
        return 'Health & Wellness'
    elif any(kw in topic_lower for kw in ['finance', 'investing', 'money', 'business']):
...
def visualize_results(results_df, similarities):
    """
    Visualize the results of the ablation test for topics
    """
    ...
    results_df['topic_category'] = results_df['topic'].apply(categorize_topic)
    ...
    plt.title('Distribution of Semantic Similarities\nWith vs Without Topic', fontsize=14)
    plt.xlabel('Cosine Similarity (higher = more similar)', fontsize=12)
    plt.ylabel('Count', fontsize=12)
    plt.legend()
    
    # Plot 2: Impact (1-similarity) by topic category
    plt.subplot(2, 2, 2)
    topic_impact = results_df.groupby('topic_category')['embedding_similarity'].agg(['mean', 'std', 'count']).reset_index()
    topic_impact['impact'] = 1 - topic_impact['mean']
    topic_impact = topic_impact.sort_values('impact', ascending=False)
    
    # Add hue column for color coding
    topic_impact['topic_category_hue'] = topic_impact['topic_category']
    
    # Plot with error bars
    sns.barplot(x='topic_category', y='impact', hue='topic_category_hue', 
                data=topic_impact, palette='viridis', legend=False, ax=plt.gca())
    
    # Add error bars
    for i, row in topic_impact.iterrows():
        plt.errorbar(i, row['impact'], yerr=row['std'], color='black', capsize=5, alpha=0.7)
    
    plt.title('Topic Impact by Category\n(Higher = More Change)', fontsize=14)
    plt.xlabel('Topic Category', fontsize=12)
    plt.ylabel('Impact (1-Similarity)', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.ylim(0, 1.0)
    
    # Plot 3: Length differences by topic categories
    plt.subplot(2, 2, 3)
    topic_length = results_df.groupby('topic_category')['length_diff_pct'].mean().reset_index()
    topic_length = topic_length.sort_values('length_diff_pct', ascending=False)
    
    # Add hue column
    topic_length['topic_category_hue'] = topic_length['topic_category']
    
    sns.barplot(x='topic_category', y='length_diff_pct', hue='topic_category_hue', 
                data=topic_length, palette='magma', legend=False)
    plt.title('Text Length Difference by Topic Category', fontsize=14)
    plt.xlabel('Topic Category', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig('topic_ablation_results.png', dpi=300)
    logger.info("Saved visualization to topic_ablation_results.png")
    ...
    plt.tight_layout()
    plt.savefig('topic_ablation_correlations.png', dpi=300)
    logger.info("Saved correlation matrix to topic_ablation_correlations.png")
...
def _resolve_input(run_id: str | None, base_dir: str, explicit_input: str | None) -> str:
    if run_id:
        ...
        if discovered and os.path.exists(discovered):
            return discovered
        return os.path.join(base_dir, run_id, "07-clean-topics.jsonl")
    if not explicit_input:
        raise ValueError("Provide --input when --run-id is not used")
    return explicit_input


def run_topic_ablation_test(run_id: str | None,
                            base_dir: str,
                            input_path: str | None,
                            sample_pct: float,
                            model_name: str,
                            ...
                            llm_batch: int,
                            embed_batch: int):
    logger.info("Starting topic micro ablation test…")

    set_global_seed(seed)

    # Resolve input and report outputs
    in_path = _resolve_input(run_id, base_dir, input_path)
    reports_dir = os.path.join("reports", run_id or "adhoc", "topic-ablation")
    os.makedirs(reports_dir, exist_ok=True)

    sig = compute_hash([in_path], {"stage": 8, "sample_pct": sample_pct, "model": model_name, "llm_batch": llm_batch, "embed_batch": embed_batch, "stage_version": STAGE_VERSION})
    if run_id:
        manifest = read_manifest(run_id, base_dir)
        out_paths = [
            os.path.join(reports_dir, "topic_ablation_results.png"),
            os.path.join(reports_dir, "topic_ablation_correlations.png"),
            os.path.join(reports_dir, "topic_ablation_details.txt"),
        ]
        if should_skip(manifest, "08-topic-ablation", sig, out_paths):
            logger.info(f"Skipping 08-topic-ablation; up-to-date in {reports_dir}")
            return
    ...
    with open(in_path, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                ...
                continue
            if not post.get('post_text') or not post.get('topic'):
                continue
            eligible += 1
    ...
    # Generate with and without topic
    with_topic_texts = generate_posts(llm, sample_posts, include_topic=True, llm_batch=llm_batch)
    without_topic_texts = generate_posts(llm, sample_posts, include_topic=False, llm_batch=llm_batch)

    valid_with_texts = [t for t in with_topic_texts if t.strip()]
    valid_without_texts = [t for t in without_topic_texts if t.strip()]
    if not valid_with_texts or not valid_without_texts:
        logger.error("No valid texts for embedding!")
        return

    with_topic_embeddings = compute_embeddings(valid_with_texts, embedding_model, embed_batch=embed_batch)
    without_topic_embeddings = compute_embeddings(valid_without_texts, embedding_model, embed_batch=embed_batch)

    similarities = []
    if len(with_topic_embeddings) > 0 and len(without_topic_embeddings) > 0:
        similarities = compute_embedding_similarities(with_topic_embeddings, without_topic_embeddings)

    # Analyze and visualize
    results_df = analyze_differences(with_topic_texts, without_topic_texts, sample_posts)
    if len(results_df) == 0:
    ...
    details_path = os.path.join(reports_dir, 'topic_ablation_details.txt')
    with open(details_path, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("TOPIC MICRO ABLATION TEST RESULTS\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"- Posts analyzed: {len(results_df)}\n")
        f.write(f"- Overall topic impact: {overall_impact:.4f} (0=none, 1=complete change)\n")
        ...
        results_df['topic_category'] = results_df['topic'].apply(categorize_topic)
        f.write("Impact by topic category:\n")
        topic_impact_df = results_df.groupby('topic_category')['embedding_similarity'].agg(['mean', 'std', 'count']).reset_index()
        topic_impact_df['impact'] = 1 - topic_impact_df['mean']
        topic_impact_df = topic_impact_df.sort_values('impact', ascending=False)
        for _, row in topic_impact_df.iterrows():
            f.write(f"- {row['topic_category']}: {row['impact']:.4f} (samples: {row['count']})\n")
        ...
        if 'embedding_similarity' in results_df.columns and len(results_df) > 0:
            ...
            for _, row in most_different.iterrows():
                f.write(f"Example (topic: {row['topic']}, similarity: {row['embedding_similarity']:.4f}):\n\n")
                with_text = with_topic_texts[row['index']] if row['index'] < len(with_topic_texts) else "N/A"
                without_text = without_topic_texts[row['index']] if row['index'] < len(without_topic_texts) else "N/A"
                f.write("WITH TOPIC:\n" + with_text + "\n\n")
                f.write("WITHOUT TOPIC:\n" + without_text + "\n\n")
                f.write("-" * 80 + "\n\n")

    # Save figures to reports
    plt.savefig(os.path.join(reports_dir, 'topic_ablation_results.png'), dpi=300)
    plt.close('all')
    # Correlations saved within visualize_results; we ensure a save here too
    plt.savefig(os.path.join(reports_dir, 'topic_ablation_correlations.png'), dpi=300)

    logger.info(f"Saved topic ablation artifacts to {reports_dir}")

    if run_id:
        manifest = read_manifest(run_id, base_dir)
        update_stage(run_id, base_dir, manifest, "08-topic-ablation", in_path, [
            os.path.join(reports_dir, 'topic_ablation_results.png'),
            os.path.join(reports_dir, 'topic_ablation_correlations.png'),
            details_path,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Topic micro-ablation using 07-clean-topics output")
    ...
    try:
        ...
        logger.error(f"Error in topic micro ablation test: {e}", exc_info=True)
