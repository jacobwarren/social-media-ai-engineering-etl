...
logger = init_pipeline_logging("phase2.clean_topics", None, "07-clean-topics")

def contains_disallowed(topic: str) -> bool:
    """
    Returns True if the topic contains any disallowed substrings or characters:
      - "..."
      - "?"
      - "_"
    ...
    """
    disallowed_substrings = ["...", "?", "_", "{", "}", "[", "]", "<", ">"]
    for substr in disallowed_substrings:
        if substr in topic:
            return True
    ...
    # Check if the topic is just "unknown" (case-insensitive, after stripping)
    if topic.strip().lower() == "unknown":
        return True

    return False

def parse_topic_field(topic_field: str) -> str:
    """
    Attempts to parse the topic field if it is a JSON string containing an object.
    If it can be parsed and contains a 'topic' key, returns that value;
    otherwise returns the original string.
    """
    try:
        parsed = json.loads(topic_field)
...
def _resolve_io(run_id: str | None, base_dir: str, input_path: str | None, output_path: str | None) -> Tuple[str, str, str | None]:
    ...
    if run_id:
        ...
        discovered = discover_input(manifest, "06-extract-topics") or discover_input(manifest, "05-balance")
        in_path = discovered or input_path
        if not in_path:
            raise ValueError("No input found: provide --input or ensure manifest contains 06-extract-topics or 05-balance output")
        out_dir = os.path.join(base_dir, run_id)
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "07-clean-topics.jsonl")
...
def clean_topics(input_path: str | None,
    ...
    signature = compute_hash([in_path], {"stage": 7, "transform": "clean-topics", "stage_version": STAGE_VERSION})
    if resolved_run:
        manifest = read_manifest(resolved_run, base_dir)
        if should_skip(manifest, "07-clean-topics", signature, [out_path]):
            logger.info(f"Skipping 07-clean-topics; up-to-date at {out_path}")
    ...
    with open(out_path, 'w', encoding='utf-8') as outfile:
        with open(in_path, 'r', encoding='utf-8') as infile:
            for line in infile:
                ...
                topic_field = post.get('topic', '')
                topic_text = parse_topic_field(topic_field)

                if contains_disallowed(topic_text):
                    removed += 1
                    continue
    ...
    if resolved_run:
        manifest = read_manifest(resolved_run, base_dir)
        update_stage(resolved_run, base_dir, manifest, "07-clean-topics", in_path, [out_path], signature,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Clean topic field strings and filter disallowed topics")
