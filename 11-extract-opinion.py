...
# Configure logging
logger = init_pipeline_logging("phase2.opinion", None, "11-extract-opinion")
...
def _build_prompt(text: str) -> str:
    task = (
        "Rewrite the main viewpoint from the following social media post in first person.\n"
        "- Do not include references to outside stats, names, or context.\n"
        "- Do not add prefaces like 'The author thinks/believes' or 'I think/believe.'\n"
        "- Simply phrase the core opinion from the user's perspective, as a direct statement.\n"
        "- Reply ONLY with the opinion\n\n"
        "**Social Media Post to Extract Opinion From**\n\n"
        f"{text}"
    )
...
def process_batch(batch: List[Dict], llm: LLM, sampling_params: SamplingParams) -> Dict[str, int]:
    ...
    for map_idx, output in enumerate(outputs):
        ...
        try:
            ...
            opinion = result_json.get("opinion", "").strip()
        except Exception:
            opinion = text
        post['opinion'] = opinion
        dist_key = 'nonempty' if opinion else 'empty'
...
def process_posts(input_path: str | None,
    ...
    if run_id:
        ...
        if not resolved_input:
            raise ValueError("No input found: provide --input or ensure manifest contains 09-extract-tone/07-clean-topics output")
        out_dir = os.path.join(base_dir, run_id)
        os.makedirs(out_dir, exist_ok=True)
        resolved_output = os.path.join(out_dir, "11-opinion.jsonl")
        signature = compute_hash([resolved_input], {"stage": 11, "model": model, "batch": batch_size, "max_len": max_model_len, "temp": temperature, "stage_version": STAGE_VERSION})
        if should_skip(manifest, "11-extract-opinion", signature, [resolved_output]):
            logger.info(f"Skipping 11-extract-opinion; up-to-date at {resolved_output}")
            return
    else:
    ...
    if run_id and manifest:
        manifest = read_manifest(run_id, base_dir)
        signature = compute_hash([resolved_input], {"stage": 11, "model": model, "batch": batch_size, "max_len": max_model_len, "temp": temperature, "stage_version": STAGE_VERSION})
        update_stage(run_id, base_dir, manifest, "11-extract-opinion", resolved_input, [resolved_output], signature, extra={"total": total})


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Extract first-person opinion using vLLM (Phase 2 compat CLI)")
    ...
    process_posts(
        ...
        output_path=args.output_path or 'step-11-posts-with-opinions.jsonl',
        run_id=args.run_id,
