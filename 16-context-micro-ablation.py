...
# Configure logging
logger = init_pipeline_logging("phase2.context_ablation", None, "16-context-micro-ablation")
...
def create_prompt(post, include_context=True):
    """
    Create prompt with or without context feature
    ...
    # Only difference is whether we include specific context or use generic wording
    if include_context and 'context' in post:
        context = post.get('context')
        first_line = f"Create a LinkedIn post on the context of: `{context}`"
    else:
        # Generic opening without specific context
        first_line = f"Create a LinkedIn post"
        
    # Build the prompt with all features identical except for context
    prompt = f"""{first_line}
...
def generate_posts(llm, posts, include_context=True, llm_batch=8, temperature=0.7, max_tokens=1000, top_p=0.9):
    """
    Generate LinkedIn posts using the LLM in batches.
    """
    logger.info(f"Generating posts with context={'included' if include_context else 'excluded'}...")
    prompts = [create_prompt(post, include_context) for post in posts]
...
def compute_embedding_similarities(with_context_embeddings, without_context_embeddings):
    """
    ...
    for i in range(min(len(with_context_embeddings), len(without_context_embeddings))):
        sim = cosine_similarity(
            with_context_embeddings[i].reshape(1, -1),
            without_context_embeddings[i].reshape(1, -1)
        )[0][0]
        similarities.append(sim)
    
    return similarities

def analyze_differences(with_context_texts, without_context_texts, posts):
    """
    Analyze differences between texts generated with and without context
    """
    logger.info("Analyzing text differences...")
    results = []
    
    for i, (with_t, without_t, post) in enumerate(zip(with_context_texts, without_context_texts, posts)):
        ...
        context = post.get('context', 'unknown')
        
        results.append({
            'index': i,
            'context': context,
            'with_context_length': with_t_length,
            'without_context_length': without_t_length,
            'length_diff_pct': length_diff_pct,
            'paragraph_diff': paragraph_diff,
            'bullet_diff': bullet_diff,
            'with_context_text': with_t[:200] + "..." if len(with_t) > 200 else with_t,
            'without_context_text': without_t[:200] + "..." if len(without_t) > 200 else without_t
        })
...
def categorize_context(context):
    """
    Categorize contexts into broader groups for visualization
    """
    context_lower = context.lower()
    if any(kw in context_lower for kw in ['marketing', 'sales', 'branding', 'advertising']):
        return 'Marketing & Sales'
    elif any(kw in context_lower for kw in ['tech', 'ai', 'software', 'data', 'programming', 'digital']):
        return 'Technology & AI'
    elif any(kw in context_lower for kw in ['leadership', 'management', 'career', 'professional']):
        return 'Leadership & Career'
    elif any(kw in context_lower for kw in ['health', 'wellness', 'fitness', 'mental']):
        return 'Health & Wellness'
    elif any(kw in context_lower for kw in ['finance', 'investing', 'money', 'business']):
...
def visualize_results(results_df, similarities):
    """
    Visualize the results of the ablation test for contexts
    """
    ...
    results_df['context_category'] = results_df['context'].apply(categorize_context)
    ...
    plt.title('Distribution of Semantic Similarities\nWith vs Without Context', fontsize=14)
    plt.xlabel('Cosine Similarity (higher = more similar)', fontsize=12)
    plt.ylabel('Count', fontsize=12)
    plt.legend()
    
    # Plot 2: Impact (1-similarity) by context category
    plt.subplot(2, 2, 2)
    context_impact = results_df.groupby('context_category')['embedding_similarity'].agg(['mean', 'std', 'count']).reset_index()
    context_impact['impact'] = 1 - context_impact['mean']
    context_impact = context_impact.sort_values('impact', ascending=False)
    
    # Add hue column for color coding
    context_impact['context_category_hue'] = context_impact['context_category']
    
    # Plot with error bars
    sns.barplot(x='context_category', y='impact', hue='context_category_hue', 
                data=context_impact, palette='viridis', legend=False, ax=plt.gca())
    
    # Add error bars
    for i, row in context_impact.iterrows():
        plt.errorbar(i, row['impact'], yerr=row['std'], color='black', capsize=5, alpha=0.7)
    
    plt.title('Context Impact by Category\n(Higher = More Change)', fontsize=14)
    plt.xlabel('Context Category', fontsize=12)
    plt.ylabel('Impact (1-Similarity)', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.ylim(0, 1.0)
    
    # Plot 3: Length differences by context categories
    plt.subplot(2, 2, 3)
    context_length = results_df.groupby('context_category')['length_diff_pct'].mean().reset_index()
    context_length = context_length.sort_values('length_diff_pct', ascending=False)
    
    # Add hue column
    context_length['context_category_hue'] = context_length['context_category']
    
    sns.barplot(x='context_category', y='length_diff_pct', hue='context_category_hue', 
                data=context_length, palette='magma', legend=False)
    plt.title('Text Length Difference by Context Category', fontsize=14)
    plt.xlabel('Context Category', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig('context_ablation_results.png', dpi=300)
    logger.info("Saved visualization to context_ablation_results.png")
    ...
    plt.tight_layout()
    plt.savefig('context_ablation_correlations.png', dpi=300)
    logger.info("Saved correlation matrix to context_ablation_correlations.png")
...
def _resolve_input(run_id: str | None, base_dir: str, explicit_input: str | None) -> str:
    if run_id:
        ...
        discovered = discover_input(manifest, "15-clean-context") or discover_input(manifest, "14-extract-context")
        if discovered and os.path.exists(discovered):
            return discovered
        return os.path.join(base_dir, run_id, "15-clean-context.jsonl")
    if not explicit_input:
        raise ValueError("Provide --input when --run-id is not used")
    return explicit_input


def run_context_ablation_test(run_id: str | None,
                              base_dir: str,
                              input_path: str | None,
                              sample_pct: float,
                              model_name: str,
                              ...
                              llm_batch: int,
                              embed_batch: int):
    logger.info("Starting micro ablation test for contexts…")

    set_global_seed(seed)

    # Resolve input and report outputs
    in_path = _resolve_input(run_id, base_dir, input_path)
    reports_dir = os.path.join("reports", run_id or "adhoc", "context-ablation")
    os.makedirs(reports_dir, exist_ok=True)

    sig = compute_hash([in_path], {"stage": 16, "sample_pct": sample_pct, "model": model_name, "llm_batch": llm_batch, "embed_batch": embed_batch, "stage_version": STAGE_VERSION})
    if run_id:
        manifest = read_manifest(run_id, base_dir)
        out_paths = [
            os.path.join(reports_dir, "context_ablation_results.png"),
            os.path.join(reports_dir, "context_ablation_correlations.png"),
            os.path.join(reports_dir, "context_ablation_details.txt"),
        ]
        if should_skip(manifest, "16-context-ablation", sig, out_paths):
            logger.info(f"Skipping 16-context-ablation; up-to-date in {reports_dir}")
            return
    ...
    with open(in_path, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                ...
                continue
            if not post.get('post_text') or not post.get('context'):
                continue
            eligible += 1
    ...
    # Generate with and without context
    with_context_texts = generate_posts(llm, sample_posts, include_context=True, llm_batch=llm_batch)
    without_context_texts = generate_posts(llm, sample_posts, include_context=False, llm_batch=llm_batch)

    valid_with_texts = [t for t in with_context_texts if t.strip()]
    valid_without_texts = [t for t in without_context_texts if t.strip()]
    if not valid_with_texts or not valid_without_texts:
        logger.error("No valid texts for embedding!")
        return

    with_context_embeddings = compute_embeddings(valid_with_texts, embedding_model, embed_batch=embed_batch)
    without_context_embeddings = compute_embeddings(valid_without_texts, embedding_model, embed_batch=embed_batch)

    similarities = []
    if len(with_context_embeddings) > 0 and len(without_context_embeddings) > 0:
        similarities = compute_embedding_similarities(with_context_embeddings, without_context_embeddings)

    # Analyze and visualize
    results_df = analyze_differences(with_context_texts, without_context_texts, sample_posts)
    if len(results_df) == 0:
    ...
    details_path = os.path.join(reports_dir, 'context_ablation_details.txt')
    with open(details_path, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("CONTEXT MICRO ABLATION TEST RESULTS\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"- Posts analyzed: {len(results_df)}\n")
        f.write(f"- Overall context impact: {overall_impact:.4f} (0=none, 1=complete change)\n")
        ...
        results_df['context_category'] = results_df['context'].apply(categorize_context)
        f.write("Impact by context category:\n")
        context_impact_df = results_df.groupby('context_category')['embedding_similarity'].agg(['mean', 'std', 'count']).reset_index()
        context_impact_df['impact'] = 1 - context_impact_df['mean']
        context_impact_df = context_impact_df.sort_values('impact', ascending=False)
        for _, row in context_impact_df.iterrows():
            f.write(f"- {row['context_category']}: {row['impact']:.4f} (samples: {row['count']})\n")
        ...
        if 'embedding_similarity' in results_df.columns and len(results_df) > 0:
            ...
            for _, row in most_different.iterrows():
                f.write(f"Example (context: {row['context']}, similarity: {row['embedding_similarity']:.4f}):\n\n")
                with_text = with_context_texts[row['index']] if row['index'] < len(with_context_texts) else "N/A"
                without_text = without_context_texts[row['index']] if row['index'] < len(without_context_texts) else "N/A"
                f.write("WITH CONTEXT:\n" + with_text + "\n\n")
                f.write("WITHOUT CONTEXT:\n" + without_text + "\n\n")
                f.write("-" * 80 + "\n\n")

    # Save figures to reports
    plt.savefig(os.path.join(reports_dir, 'context_ablation_results.png'), dpi=300)
    plt.close('all')
    plt.savefig(os.path.join(reports_dir, 'context_ablation_correlations.png'), dpi=300)

    logger.info(f"Saved context ablation artifacts to {reports_dir}")

    if run_id:
        manifest = read_manifest(run_id, base_dir)
        update_stage(run_id, base_dir, manifest, "16-context-ablation", in_path, [
            os.path.join(reports_dir, 'context_ablation_results.png'),
            os.path.join(reports_dir, 'context_ablation_correlations.png'),
            details_path,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Context micro-ablation using 15-clean-context output")
    ...
    try:
        ...
        logger.error(f"Error in context micro ablation test: {e}", exc_info=True)
